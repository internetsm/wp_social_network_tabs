...
function dc_social_tabs_shortcode( $atts, $content = null ){
	...
	foreach($order as $function){
		...
		if($function != '' && $options[$function.'_inc'] == 'true'){
			...
			if($function == 'pinterest' && $p1 == 0){
				$params .= 'url: "'.plugins_url() . '/social-network-tabs/inc/dcwp_rss.php",';
				$p1 = 1;
			}
			
			if($function == 'rss' && $r1 == 0){
				$params .= 'url: "'.plugins_url() . '/social-network-tabs/inc/dcwp_rss.php",';
				$r1 = 1;
			}
				...
				/*else if($function == 'rss' && $k == 'retweets'){
					$params .= $j > 0 ? ','.$k.': '.$v : $k.': '.$v ;
				} 
				*/
	...
	$out .= 'if(!jQuery().dcSocialTabs) { $.getScript("'.dc_jqsocialtabs::get_plugin_directory().'/js/jquery.social.media.tabs.1.7.4.min.js", function(){$("#social-tabs").dcSocialTabs(config);}); } else {';
